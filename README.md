...
- **Sound preview** — CodeLens on `SMODS.Sound { key, path }` and `play_sound('key', …)` calls. Webview with pitch + volume sliders (real pitch shift, duration preserved), loop toggle, and a copyable `play_sound(...)` snippet.
...
| `Preview Sound…` | Opened from the CodeLens above any `SMODS.Sound { key, path }` block or `play_sound('key', …)` call site. Embedded audio player with pitch + volume sliders, loop toggle, and a live `play_sound(...)` snippet copy. |
